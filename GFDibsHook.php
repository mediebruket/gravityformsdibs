...
class GFDibsHook
{
    ...
    public static function dibsTransition($confirmation, $form, $lead, $ajax)
    {
        ...
        $order_id = _is($_POST, 'orderId');
        if (!$order_id && $feed_id = $Dao->isDibsForm($form['id'])) {
            $feed = $Dao->getDibsMeta($feed_id);
            //$Dao->log('feed:');
            //$Dao->log($feed);
            ...
            if (isset($feed->meta['gf_dibs_type'])) {
                if ($feed->meta['gf_dibs_type'] == '2') {
                    ...
                    $_POST['captureNow'] = '1';
                } elseif ($feed->meta['gf_dibs_type'] == '3') {
        ...
        //$Dao->log($_POST);
            ...
            foreach ($_POST as $key => $value) {
                if (
              // !is_numeric(strpos($key, 'input')) &&
              !is_numeric(strpos($key, 'MAX_FILE_SIZE'))
              && !is_numeric(strpos($key, 'state'))
              && !is_numeric(strpos($key, 'gform'))) {
    ...
    */
    ...
    public static function grabOrderId($post)
    {
        ...
        } elseif (isset($post['orderid']) && strlen($post['orderid'])) {
            $order_id = $post['orderid'];
        }
    ...
    public static function formTag($form_tag, $form)
    {
        ...
        if (isset($_POST['orderId'])) {
            ...
            if ($Transaction = $Dao->getTransactionByOrderId($_POST['orderId'])) {
                ...
                // get and handle confirmation message
                _log('lead id');
                _log(_is($_POST, 'leadId'));

                // try to get confirmation by lead
                if ($lead = GFAPI::get_entry(_is($_POST, 'leadId'))) {
                    _log('lead');
                    _log($lead);
                    $message = GFFormDisplay::handle_confirmation($form, $lead);
                    _log('$message');
                    _log($message);
                }
                // else get default confirmation
                else {
    ...
    <tbody>
      <tr>
        <td colspan="2" class="entry-view-field-name"><?php _e('Order id:'); ?> <span><?php if (isset($Transaction->order_id)) {
        echo $Transaction->order_id;
    } ?></span></td>
      </tr>
      <tr>
